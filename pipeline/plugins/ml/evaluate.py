...
def compare_runs(run_id_a: str, run_id_b: str) -> str:
    ...
    Returns a formatted comparison table with deltas and a recommendation
    ...
    if not metrics_a:
        return f"Run {run_id_a} has no eval_ metrics. Run evaluation first."
    ...
    wf1_a = metrics_a.get("eval_weighted_f1", 0.0)
    wf1_b = metrics_b.get("eval_weighted_f1", 0.0)
    if wf1_b > wf1_a:
